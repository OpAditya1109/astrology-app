...
export default function YantraPage() {
  // Filter only Yantra products
  const yantraProducts = products.filter((p) => p.category === "yantra");
  ...
  return (
    <div className="min-h-screen bg-yellow-50 p-6">
      ...
      <p className="text-gray-600 text-center mb-8">
        Explore our sacred Yantras for spiritual growth, protection, and energy.
      </p>

      {/* Products Grid */}
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-8 max-w-6xl mx-auto">
        {yantraProducts.map((product) => (
          <Link
            key={product.id}
            to={`/product/${product.id}`} // links to same ProductDetail page
          ...
          >
            ...
            <div className="mt-1 flex items-center gap-2">
              ...
              <span className="text-yellow-700 font-bold">{product.price}</span>
            </div>
          </Link>
        ))}
      </div>
